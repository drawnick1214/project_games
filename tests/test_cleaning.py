...
def test_standardize_columns(raw_df):
    ...
    assert "year_of_release" in result.columns
...
def test_fill_year_from_name(raw_df):
    ...
    # "FIFA 2014" should get year 2014 extracted from name
...
def test_drop_incomplete_rows(raw_df):
    ...
    # Row with None name (index 3) and row with None genre (index 3) should be dropped
...
def test_drop_duplicates():
    ...
    # Should keep the row with higher total_sales
    assert len(result) == 2
